 import numpy as np
import matplotlib.pyplot as plt

def bresenham_line(x0, y0, x1, y1):
    xes, yes = [], []
    dx = abs(x1 - x0)
    dy = abs(y1 - y0)
    sx = 1 if x1 >= x0 else -1
    sy = 1 if y1 >= y0 else -1
    x, y = x0, y0

    if dx >= dy:
        p = 2 * dy - dx
        for _ in range(dx + 1):
            xes.append(x)
            yes.append(y)
            x += sx
            if p >= 0:
                y += sy
                p += 2 * dy - 2 * dx
            else:
                p += 2 * dy
    else:
        p = 2 * dx - dy
        for _ in range(dy + 1):
            xes.append(x)
            yes.append(y)
            y += sy
            if p >= 0:
                x += sx
                p += 2 * dx - 2 * dy
            else:
                p += 2 * dx

    return np.array(xes), np.array(yes)


def apply_2d_transformation(x_coords, y_coords, matrix):
    points = np.vstack([x_coords, y_coords, np.ones_like(x_coords)])
    transformed = matrix @ points
    return transformed[0], transformed[1]


def midpoint_scaling(x0, y0, x1, y1, Sx, Sy):

    x_orig, y_orig = bresenham_line(x0, y0, x1, y1)

    xm = (x0 + x1) / 2
    ym = (y0 + y1) / 2

    T_to_origin = np.array([
        [1, 0, -xm],
        [0, 1, -ym],
        [0, 0,  1]
    ])

    S = np.array([
        [Sx, 0,  0],
        [0,  Sy, 0],
        [0,  0,  1]
    ])

    T_back = np.array([
        [1, 0, xm],
        [0, 1, ym],
        [0, 0,  1]
    ])

    composite_matrix = T_back @ S @ T_to_origin

    x_scaled, y_scaled = apply_2d_transformation(
        x_orig, y_orig, composite_matrix
    )


    plt.figure(figsize=(8, 6))
    plt.plot(x_orig, y_orig, marker='o',
             linestyle='-', label='Original Line')
    plt.plot(x_scaled, y_scaled, marker='x',
             linestyle='--', label='Midpoint Scaled Line')

    plt.scatter(xm, ym)
    plt.text(xm, ym, "  Midpoint")

    plt.title("Scaling About Midpoint")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    plt.grid(True)
    plt.axis('equal')
    plt.show()

midpoint_scaling(2, 3, 10, 8, 2, 0.5)